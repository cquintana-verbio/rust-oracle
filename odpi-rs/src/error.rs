...
use crate::util::ptr_to_rust_str;
use crate::util::to_rust_str;
use crate::util::AssertSend;
use crate::util::AssertSync;
use odpi_sys::dpiErrorInfo;
use std::error;
use std::fmt;
use std::num;
use std::str;
...
impl Error {
    pub unsafe fn with_raw_err(err: &dpiErrorInfo) -> Error {
        let err = DbError::new(
            err.code,
            err.offset,
            to_rust_str(err.message, err.messageLength),
            ptr_to_rust_str(err.fnName),
            ptr_to_rust_str(err.action),
        );
